function [J, gradVec] = ResidualObjectiveVecStable(qVec, fields, params, phase, dx, dy, sz, X, Y, maxGrad)
    if nargin < 10
        maxGrad = 1; % по умолчанию ограничение градиента
    end

    % --- 1. Распаковать параметры ---
    ...
    if ~isfinite(J)
        error('ResidualObjectiveVecStable: J is not finite!');
    end
    ...
    % --- 7. Ограничение градиентов ---
    gradUx = max(min(gradUx, maxGrad), -maxGrad);
    gradUy = max(min(gradUy, maxGrad), -maxGrad);
    gradT  = max(min(gradT,  maxGrad), -maxGrad);
    gradP  = max(min(gradP,  maxGrad), -maxGrad);

    % --- 8. Упаковать градиенты ---
    gradVec = packParams(gradUx, gradUy, gradT, gradP);

    % --- 9. Отладочный вывод ---
    % disp(['J = ', num2str(J), ', max grad = ', num2str(max(abs(gradVec)))]);
end
