...
%%
function val = QT_dQT_dux(Q_T, fields, params, phase, dx, dy)

    % 19.09.2025, rewritten by Mark 
    % Реализует: ∑∑ Q_T^(i1,j1) * (∂Q_T^(i1,j1))/(∂u_x^(i,j))
    ...
    % ---------- Вклад по x (аналогично d/dy в QT_dQT_duy) ----------
    % левая сторона (i-1)
    i = 1:M-2; j = 2:N-1;
    val(i,j) = val(i,j) - Q_T(i+1,j) * (4/3) * mu(i+1,j) * ( ...
                  ( (u_x(i+2,j) - u_x(i,j)) / (2*dx^2) + ...
                    (u_y(i+1,j+1) - u_y(i+1,j-1)) / (2*dx*dy) ) );

    % правая сторона (i+1)
    i = 3:M; j = 2:N-1;
    val(i,j) = val(i,j) + Q_T(i-1,j) * (4/3) * mu(i-1,j) * ( ...
                  ( (u_x(i,j) - u_x(i-2,j)) / (2*dx^2) + ...
                    (u_y(i-1,j+1) - u_y(i-1,j-1)) / (2*dx*dy) ) );

    % ---------- Вклад по y (аналогично d/dx в QT_dQT_duy) ----------
    % нижняя сторона (j-1)
    i = 2:M-1; j = 1:N-2;
    val(i,j) = val(i,j) + Q_T(i,j+1) * 2 * mu(i,j+1) / (2*dy) * ( ...
                  (u_y(i+1,j+1) - u_y(i-1,j+1)) / (2*dx) + ...
                  (u_x(i,j+2) - u_x(i,j)) / (2*dy) );

    % верхняя сторона (j+1)
    i = 2:M-1; j = 3:N;
    val(i,j) = val(i,j) - Q_T(i,j-1) * 2 * mu(i,j-1) / (2*dy) * ( ...
                  (u_y(i+1,j-1) - u_y(i-1,j-1)) / (2*dx) + ...
                  (u_x(i,j) - u_x(i,j-2)) / (2*dy) );

    % ---------- Конвективный вклад ----------
    i = 2:M-1; j = 2:N-1;
    val(i,j) = val(i,j) + Q_T(i,j) * rho(i,j) * cp(i,j) * ...
                 (T(i+1,j) - T(i-1,j)) / (2*dx);
